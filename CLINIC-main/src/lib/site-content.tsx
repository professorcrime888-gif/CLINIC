...
import { contact, doctor, clinicName as defaultClinicName, services as defaultServicesData, workingHours as defaultWorkingHours } from '@/data/clinic'
...
export interface SiteContent {
  ...
  specialty: string
  clinicName: string
  /** Image URL or data-URL for the logo. Empty = initials badge ("عأ"). */
  logoUrl: string
  /** Hero section illustration. Empty = default illustration. */
  heroImageUrl: string
  /** "لماذا تختارين عيادتنا" section illustration. Empty = default illustration. */
...
export const defaultSiteContent: SiteContent = {
  doctorName: '4D Fetal Care',
  specialty: doctor.specialty,
  clinicName: defaultClinicName,
  logoUrl: '/img/logo.png',
  heroImageUrl: '/img/hero-illustration.png',
  aboutImageUrl: '/img/clinic-illustration.png',
...
/**
 * Resolves an image field from site content into a usable <img src>.
 * - Empty -> falls back to the given default path.
 * - Data URLs / absolute http(s) URLs (uploaded from the admin panel, or a
 *   pasted external link) are used as-is.
 * - Local /img/... paths go through Netlify's image CDN for resizing/webp.
 */
export function resolveImageSrc(value: string | undefined, fallbackPath: string, width = 900): string {
  const src = value && value.trim().length > 0 ? value.trim() : fallbackPath
  if (src.startsWith('data:') || src.startsWith('http://') || src.startsWith('https://')) {
    return src
  }
  return `/.netlify/images?url=${src}&w=${width}&fm=webp`
}

/** Initials fallback badge (e.g. "عأ") computed from a full name, used when no logo is set. */
