    ...
    <footer id="contact" className="border-t border-slate-100 bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <div className="mb-3 flex items-center gap-3">
            <Logo name={content.doctorName} logoUrl={content.logoUrl} size={40} />
            <span className="text-sm font-bold text-slate-900">{content.doctorName}</span>
          </div>
          <p className="text-sm leading-relaxed text-slate-600">{content.clinicName}</p>
          <p className="mt-1 text-sm text-teal-700">{content.specialty}</p>
        </div>
      ...
      <div className="border-t border-slate-200">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-5 py-5 text-xs text-slate-500 sm:flex-row">
          <span>
            © {year} {content.clinicName}. جميع الحقوق محفوظة.
          </span>
