...
import { defaultSiteContent, applyThemeColor, resolveImageSrc, type SiteContent } from '@/lib/site-content'
      ...
      <form onSubmit={handleSave} className="mt-8 space-y-8">
        <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-bold text-slate-900">
            ...
            هوية الطبيب والعيادة
          </h2>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field
              label="العنوان الرئيسي (اسم الطبيب أو اسم البراند)"
              value={form.doctorName}
              onChange={(v) => setForm({ ...form, doctorName: v })}
            />
            <Field
              label="العنوان الفرعي (التخصص أو تسمية البراند)"
              value={form.specialty}
              onChange={(v) => setForm({ ...form, specialty: v })}
            />
            <Field
              label="اسم العيادة (يظهر في الفوتر وحقوق النشر)"
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            ...
            />
            <ImageField
              label={'صورة قسم "لماذا تختارين عيادتنا"'}
              value={form.aboutImageUrl}
              onChange={(v) => setForm({ ...form, aboutImageUrl: v })}
            />
          </div>
          <p className="mt-3 text-xs text-slate-400">
            الحقلين الأول والثاني هما أول وأكبر نص يظهر في الصفحة الرئيسية وأعلى الموقع — اكتبي
            فيهم اسم الطبيب، أو اسم البراند (مثل 4D Fetal Care)، أو أي نص تحبي تظهريه. الشعار لما
            يترفع بيحل محل حروف "عأ" تلقائيًا في كل الموقع.
          </p>
        </section>
...
/** Lets the admin either upload an image (stored as a data-URL) or paste a direct image link. */
function ImageField({
  label,
  value,
  onChange,
  ...
  function handleFile(file: File | undefined) {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setError('الصورة كبيرة جدًا، افضّل صورة أصغر من 4 ميجا.')
      return
    }
    const reader = new FileReader()
    reader.onload = () => onChange(String(reader.result))
    reader.onerror = () => setError('تعذّر قراءة الصورة، حاول مرة أخرى.')
  ...
  return (
    <div className="text-sm">
      ...
      <div className="flex items-center gap-3">
        <div
          className={`flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden border border-slate-200 bg-slate-50 ${
            previewRound ? 'rounded-2xl' : 'rounded-xl'
          }`}
        >
          {value ? (
            <img src={resolveImageSrc(value, value, 200)} alt="" className="h-full w-full object-contain" />
          ) : (
        ...
        <div className="flex-1 space-y-2">
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) => onChange(e.target.value)}
              placeholder={value.startsWith('data:') ? 'تم رفع صورة من الجهاز' : 'الصق رابط صورة، أو ارفع من جهازك'}
              className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none focus:border-teal-700"
            />
            {value && (
              <button
                type="button"
                onClick={() => onChange('')}
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 text-slate-400 hover:border-red-200 hover:text-red-600"
